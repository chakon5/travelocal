...
<div class="grid-container">
    ...
    <button class="button button3" onclick="location='{{ route('createGuide') }}'">
    ...
    @endif
        ...
        <tbody class="table-hover">
            ...
            <tr>
                ...
                <td class="text-left">
                    @if ($row->guide_status==1)
                        <a href="{{ url('change-statusguide/'.$row->guide_id) }}" onclick="return confirm('ต้องการอัพเดทสถานะใช่หรือไม่ ?')" class="btn btn-sm btn-success"> จองแล้ว </a>
                    @else
                        <a href="{{ url('change-statusguide/'.$row->guide_id) }}" onclick="return confirm('ต้องการอัพเดทสถานะใช่หรือไม่ ?')" class="btn btn-sm btn-danger"> ว่างงาน </a>
                    @endif
                </td>
                {{-- <td class="text-left">{{Carbon\Carbon::parse($row->created_at)->diffForHumans()}}</td> --}}

                <td class="text-center">
                    <a href="{{url('/guide/edit/'.$row->guide_id)}}" class="btn btn-warning"><i class="fa-solid fa-pen-to-square"></i></a>
                    <a href="{{url('/guide/delete/'.$row->guide_id)}}" class="btn btn-danger"><i class="fa-solid fa-trash"></i></a>
                </td>
            </tr>
            @endforeach
        </tbody>
